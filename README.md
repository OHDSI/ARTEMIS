...
For convenience, set the `ARTEMIS_PY_VERSION` system variable to the correct Python path. If unsure how to do this, refer to the OS-specific setup instructions below. Make sure you have devtools installed. You can install it from within R:
...
Then open cmd or PowerShell and set the environment variable:
    ...
    set ARTEMIS_PY_VERSION=<ABS\\PATH\\TO\\python.exe (v3.12+)>
...
Then from the terminal, set the Python version environment variable:
    ...
    export ARTEMIS_PY_VERSION="/absolute/path/to/python3.12"
