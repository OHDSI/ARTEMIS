...
calculateEras <- function(pa, discontinuationTime = 120) {
    ...
    if (length(patients) > 1) {
        ...
        pa_s <- calculateEras(pa_s, discontinuationTime = discontinuationTime)
    ...
    # TODO: check this part - removing overlapping regimens just if time overlaps   
    if (nrow(pa) > 1) {
    ...
    pa <- pa %>%
        dplyr::mutate(
            lag = dplyr::lag(timeToNextRegimen),
            delete = ifelse((
                dplyr::lag(timeToNextRegimen) < discontinuationTime &
