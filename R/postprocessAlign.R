...
#' Adds line of treatment era data to a processed regimen alignment ouput
...
#' @importFrom dplyr row_number lag case_when mutate group_by summarise lead arrange
lineOfTreatment <- function(pa, drugDF, discontinuationTime = 120) {
    ...
    pa <- pa %>%
        arrange(t_start) %>%
        mutate(timeToNextRegimen = pmax(0, dplyr::lead(t_start, default = 0) - t_start)) %>%
        mutate(interval = pmax(0, dplyr::lead(t_start, default = 0) - t_end)) %>%
    ...
    pa <- pa %>%
        ...
        summarise(
            ...
            .by = c(component, lines, personID, DrugRecord_full)
        ) %>%
        mutate(
